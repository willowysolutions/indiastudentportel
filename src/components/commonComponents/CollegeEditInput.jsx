...
const InputEdit= ({ label, type, id, register, errors }) => {
  ...
  return (
    <div className="relative mb-3">
     ...
     <label
                htmlFor={id}
                className={`pointer-events-none px-2 py-0 absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.67rem] leading-[1.6] text-blue-500 transition-all duration-200 ease-out -translate-y-[0.9rem] scale-[0.8] text-primary bg-white pt-0'  motion-reduce:transition-none`}
            >
                {label}
            </label>
            <div className="text-red-500">{errors && errors[id] && errors[id].message}</div>
    </div>
  );
};
